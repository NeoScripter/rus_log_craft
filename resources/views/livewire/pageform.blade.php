        ...
        <form wire:submit.prevent="submitForm" class="space-y-4 md:flex-1 lg:space-y-6">
            ...
            <div class="relative">
                ...
                <input wire:model.blur="full_name" type="text" id="name-page"
                    class="w-full px-6 py-5 input-field" placeholder="{{ __('livewire/pageform.Ваше ФИО') }}"
                    required>
                @error('full_name')
            ...
            <div class="relative">
                ...
                <textarea wire:model.blur="message" id="message-page" rows="4"
                    class="w-full px-6 py-5 input-field" placeholder="{{ __('livewire/pageform.Сообщение') }}" required></textarea>
                @error('message')
                    <span class="absolute block text-sm text-red-600">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="relative">
                <div class="flex items-start gap-6 me-4">
                    ...
                    <label for="checkbox" class="text-sm font-bold text-gray-900">{{ __('livewire/pageform.Нажимая кнопку отправить') }}</label>
                </div>
                @error('checkbox')
                    <span class="absolute block text-sm text-red-600">{{ $message }}</span>
                @enderror
